  ...
  <div class="ledger-upload">
    ...
    <div class="content-area">
      ...
      <div v-if="currentStep === 0" class="step-content">
        <h2 class="step-title">选择销售合同</h2>
        
        <!-- 搜索选择区 -->
        <div class="section">
          <div class="section-header">
            <h3 class="section-title">销售合同</h3>
      ...
      <div v-if="currentStep === 2" class="step-content">
        <h2 class="step-title">选择销售结算单</h2>
        
        <!-- 搜索选择区 -->
        <div class="section">
          <div class="section-header">
            <h3 class="section-title">销售结算单</h3>
            <div class="search-box">
              <div class="search-input-wrapper">
                <i class="search-icon">🔍</i>
                <input 
      ...
      <div v-if="currentStep === 3" class="step-content">
        ...
        <div class="section">
          <div class="section-header">
            <h3 class="section-title">销售发票</h3>
            <div class="search-box">
              <div class="search-input-wrapper">
                <i class="search-icon">🔍</i>
                <input 
                  type="text" 
                  v-model="salesInvoiceSearchKey"
                  placeholder="搜索销售发票"
                  class="search-input"
                  @keyup.enter="handleSearch('salesInvoice')"
                  @input="handleSearchInput('salesInvoice')"
                />
                <i 
                  v-if="salesInvoiceSearchKey" 
                  class="clear-icon"
                  @click="clearSearch('salesInvoice')"
                >
                  ✕
                </i>
              </div>
          ...
          <div class="list-container">
            <div 
              ...
              :key="invoice.id"
              :class="['list-item', { selected: selectedSalesInvoice?.id === invoice.id }]"
            >
              <div class="item-content">
                <div class="item-row">
                  <span class="label">销售发票名称：</span>
      ...
      <div class="selected-info" v-if="hasSelectedItems">
        ...
        <div class="selected-info-content" v-show="isSelectedInfoExpanded">
          <div class="list-container">
            ...
            <!-- 发票 -->
            <div v-if="selectedSalesInvoice" class="list-item selected">
              <div class="item-content">
                <div class="item-row">
                  <span class="label">销售发票名称：</span>
                  <span class="value">{{ selectedSalesInvoice.name }}</span>
                </div>
                <div class="item-row">
                  <span class="label">采购方：</span>
                  <span class="value">{{ selectedSalesInvoice.buyer }}</span>
                </div>
                <div class="item-row">
                  <span class="label">销售方：</span>
                  <span class="value">{{ selectedSalesInvoice.seller }}</span>
                </div>
                <div class="item-row">
                  <span class="label">金额：</span>
                  <span class="value">{{ selectedSalesInvoice.amount }}</span>
                </div>
              </div>
              <div class="item-actions">
                <button 
                  class="action-btn secondary"
                  @click="selectSalesInvoice(null)"
                >
                  取消选择
                </button>
              </div>
...
const salesInvoiceSearchKey = ref('');
...
const showSalesInvoiceDropdown = ref(false);
const showTransportInvoiceDropdown = ref(false);
const showTransferRecordDropdown = ref(false);

// 过滤后的数据
const filteredContracts = computed(() => {
  if (!contractSearchKey.value) return salesContracts.value;
  const key = contractSearchKey.value.toLowerCase();
  return salesContracts.value.filter(contract => 
...
const filteredSettlements = computed(() => {
  if (!settlementSearchKey.value) return salesSettlements.value;
  const key = settlementSearchKey.value.toLowerCase();
  return salesSettlements.value.filter(settlement => 
...
const filteredSalesInvoices = computed(() => {
  ...
  if (!salesInvoiceSearchKey.value) return salesInvoices.value;
  const key = salesInvoiceSearchKey.value.toLowerCase();
  return salesInvoices.value.filter(invoice => 
...
const selectSalesInvoice = (invoice: any) => {
...
// 提交
// 实际使用时需要调用 salesLedgerApi.createLedger(ledgerData, tenantType) 创建销售台账
const handleSubmit = async () => {
  try {
    ...
    console.log('提交台账信息：', {
      ...
      salesInvoice: selectedSalesInvoice.value,
...
const closeDropdowns = (event: MouseEvent) => {
  ...
  if (!target.closest('.search-box')) {
    ...
    showSalesInvoiceDropdown.value = false;
...
const sortConfig = ref({
  ...
  salesInvoice: { key: '', direction: 'asc' },
