...
public interface SalesLedgerService {
    // 创建台账记录
    void createLedger(SalesLedgerDTO ledgerDTO, String tenantType);
    
    // 更新台账记录
    void updateLedger(SalesLedgerDTO ledgerDTO, String tenantType);
    
    // 删除台账记录
    void deleteLedger(Long id, String tenantType);
    
    // 获取台账记录详情
    SalesLedgerVO getLedgerDetail(Long id, String tenantType);
    
    // 获取所有台账
    List<SalesLedgerVO> getAllLedgers(String tenantType);
    
    // 根据字段名和值查询台账
    List<SalesLedgerVO> getLedgersByField(String fieldName, String fieldValue, String tenantType);
    
    // 计算税额
    BigDecimal calculateTaxAmount(BigDecimal priceWithoutTax, BigDecimal taxRate);
    
    // 根据销售时间范围查询台账
    List<SalesLedgerVO> getLedgersBySaleTimeRange(LocalDateTime startTime, LocalDateTime endTime, String tenantType);
    ...
    List<SalesLedgerVO> searchLedgers(String keyword, String tenantType);

    // 根据字段值分组查询台账
    Map<String, List<SalesLedgerVO>> getLedgersGroupByField(String fieldName, String tenantType);
} 