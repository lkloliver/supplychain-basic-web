<template>
    <div class="operation-page">
      <div class="operation-header">
        <router-link :to="{ name: 'SalesLedger' }" class="back-btn">
          <ArrowLeftIcon class="back-icon" />
          <span>返回</span>
        </router-link>
        <h2 class="operation-title">销售台账</h2>
      </div>
      
      <div class="operation-tabs">
        <router-link 
          :to="{ name: 'SalesLedgerUpload' }" 
          custom
          v-slot="{ isActive }"
        >
          ...
          <div class="form-group">
            ...
            <div class="detail-fields">
              <div class="detail-field">
                <label for="field-ledgerNo">台账编号</label>
                <input 
                  type="text" 
                  id="field-ledgerNo" 
                  v-model="uploadForm.details.ledgerNo" 
                  placeholder="请输入台账编号" 
                />
              </div>
              <div class="detail-field">
              ...
              <div class="detail-field">
                <label for="field-totalAmount">总金额</label>
                <input 
                  type="number" 
                  id="field-totalAmount" 
                  v-model="uploadForm.details.totalAmount" 
                  placeholder="请输入总金额" 
                />
              </div>
              <div class="detail-field">
                <label for="field-receivedAmount">已收金额</label>
                <input 
                  type="number" 
                  id="field-receivedAmount" 
                  v-model="uploadForm.details.receivedAmount" 
                  placeholder="请输入已收金额" 
                />
              </div>
              <div class="detail-field">
  ...
  const recognizeDocument = () => {
    ...
    // 模拟识别结果
    setTimeout(() => {
      const totalAmount = Math.floor(Math.random() * 1000000);
      const receivedAmount = Math.floor(Math.random() * totalAmount);
  ...
  const submitUpload = () => {
    ...
    // 上传成功后跳转到管理页面
    router.push({ name: 'SalesLedgerManage' });
  };
  </script>
