<template>
    <div class="operation-page">
      <div class="operation-header">
        <router-link :to="{ name: 'TransportInvoice' }" class="back-btn">
          <ArrowLeftIcon class="back-icon" />
          <span>返回</span>
        </router-link>
        <h2 class="operation-title">运输收票</h2>
      </div>
      
      <div class="operation-tabs">
        <router-link 
          :to="{ name: 'TransportInvoiceUpload' }" 
          custom
          v-slot="{ isActive }"
        >
        ...
        <router-link 
          :to="{ name: 'TransportInvoiceManage' }" 
          custom
          ...
          <div class="form-group">
            ...
            <div class="detail-fields">
              <div class="detail-field">
                <label for="field-invoiceNo">发票号码</label>
                <input 
                  type="text" 
                  id="field-invoiceNo" 
                  v-model="uploadForm.details.invoiceNo" 
                  placeholder="请输入发票号码" 
                />
              </div>
              <div class="detail-field">
              ...
              <div class="detail-field">
                <label for="field-amount">发票金额</label>
                <input 
                  type="number" 
                  id="field-amount" 
              ...
              <div class="detail-field">
                <label for="field-invoiceDate">开票日期</label>
                <input 
                  type="date" 
                  id="field-invoiceDate" 
                  v-model="uploadForm.details.invoiceDate" 
                />
              </div>
              <div class="detail-field">
                <label for="field-taxRate">税率</label>
                <select id="field-taxRate" v-model="uploadForm.details.taxRate">
  ...
  const recognizeDocument = () => {
    ...
    setTimeout(() => {
      uploadForm.details = {
        invoiceNo: Math.floor(Math.random() * 10000000000).toString().padStart(10, '0'),
        transporter: '自动识别运输商',
