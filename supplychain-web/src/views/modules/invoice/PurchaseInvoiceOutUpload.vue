<template>
    <div class="operation-page">
      <div class="operation-header">
        <router-link :to="{ name: 'PurchaseInvoiceOut' }" class="back-btn">
          <ArrowLeftIcon class="back-icon" />
          <span>返回</span>
        </router-link>
        <h2 class="operation-title">采购反向开票</h2>
      </div>
      
      <div class="operation-tabs">
        <router-link 
          :to="{ name: 'PurchaseInvoiceOutUpload' }" 
          custom
          v-slot="{ isActive }"
        >
        ...
        <router-link 
          :to="{ name: 'PurchaseInvoiceOutManage' }" 
          custom
  ...
  const submitUpload = () => {
    ...
    // 上传成功后跳转到管理页面
    router.push({ name: 'PurchaseInvoiceOutManage' });
  };
  </script>
