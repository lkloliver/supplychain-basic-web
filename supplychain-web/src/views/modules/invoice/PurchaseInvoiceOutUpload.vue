<template>
  <DocumentUploadTemplate
    ...
    uploadRouteName="/dashboard/invoice/purchase-out/upload"
    manageRouteName="/dashboard/invoice/purchase-out/manage"
...
const handleRecognize = async (file: File) => {
  ...
  return new Promise((resolve) => {
    setTimeout(() => {
      // 模拟识别结果
      const result = {
        invoiceNo: 'PO-' + new Date().getFullYear() + '-' + Math.floor(Math.random() * 1000).toString().padStart(3, '0'),
        supplier: '自动识别供应商',
...
const handleSubmit = (formData: { name: string, file: File, details: Record<string, any> }) => {
  console.log('提交采购销项发票数据', formData);
