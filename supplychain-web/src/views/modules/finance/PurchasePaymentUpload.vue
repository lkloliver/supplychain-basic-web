<template>
    <div class="operation-page">
      <div class="operation-header">
        <router-link :to="{ name: 'PurchasePayment' }" class="back-btn">
          <ArrowLeftIcon class="back-icon" />
          <span>返回</span>
        </router-link>
        <h2 class="operation-title">采购付款记录</h2>
      </div>
      
      <div class="operation-tabs">
        <router-link 
          :to="{ name: 'PurchasePaymentUpload' }" 
          custom
          v-slot="{ isActive }"
        >
  ...
  const recognizeDocument = () => {
    ...
    setTimeout(() => {
      uploadForm.details = {
        paymentNo: 'PP-' + new Date().getFullYear() + '-' + Math.floor(Math.random() * 1000).toString().padStart(3, '0'),
        supplier: '自动识别供应商',
  ...
  const submitUpload = () => {
    ...
    // 上传成功后跳转到管理页面
    router.push({ name: 'PurchasePaymentManage' });
  };
  </script>
