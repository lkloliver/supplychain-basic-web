<template>
    <div class="operation-page">
      <div class="operation-header">
        <router-link :to="{ name: 'InboundQuality' }" class="back-btn">
          <ArrowLeftIcon class="back-icon" />
          <span>返回</span>
        </router-link>
        <h2 class="operation-title">入库质检单</h2>
      </div>
      
      <div class="operation-tabs">
        <router-link 
          :to="{ name: 'InboundQualityUpload' }" 
          custom
          v-slot="{ isActive }"
        >
    ...
    setTimeout(() => {
      uploadForm.details = {
        qualityNo: 'IQ-' + new Date().getFullYear() + '-' + Math.floor(Math.random() * 1000).toString().padStart(3, '0'),
        supplier: '自动识别供应商',
        productName: '自动识别产品',
