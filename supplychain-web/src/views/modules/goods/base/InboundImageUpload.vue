<template>
    <div class="operation-page">
      <div class="operation-header">
        <router-link :to="{ name: 'InboundImage' }" class="back-btn">
          <ArrowLeftIcon class="back-icon" />
          <span>返回</span>
        </router-link>
        <h2 class="operation-title">货物入库影像</h2>
      </div>
      
      <div class="operation-tabs">
        <router-link 
          :to="{ name: 'InboundImageUpload' }" 
          custom
          v-slot="{ isActive }"
        >
        ...
        <div class="upload-form">
          ...
          <div class="form-group">
            ...
            <div class="file-upload">
              <input type="file" id="doc-file" @change="handleFileChange" class="file-input" accept="image/*" />
              <label for="doc-file" class="file-label">
                <UploadCloudIcon class="upload-icon" />
                <span>{{ uploadForm.file ? uploadForm.file.name : '点击上传图片' }}</span>
              </label>
            </div>
          </div>
          ...
          <div class="form-group">
            ...
            <div class="detail-fields">
              <div class="detail-field">
                <label for="field-imageNo">影像编号</label>
                <input 
                  type="text" 
                  id="field-imageNo" 
                  v-model="uploadForm.details.imageNo" 
                  placeholder="请输入影像编号" 
                />
              </div>
              <div class="detail-field">
              ...
              <div class="detail-field">
                <label for="field-captureDate">拍摄日期</label>
                <input 
                  type="date" 
                  id="field-captureDate" 
                  v-model="uploadForm.details.captureDate" 
                />
              </div>
              <div class="detail-field">
    ...
    setTimeout(() => {
      uploadForm.details = {
        imageNo: 'II-' + new Date().getFullYear() + '-' + Math.floor(Math.random() * 1000).toString().padStart(3, '0'),
        supplier: '自动识别供应商',
        productName: '自动识别产品',
